...
std::unique_ptr<Install> Install::acquireMatch(const QString& installPath)
{
    ...
    {
        Entry entry = i.value();
        std::unique_ptr<Install> possibleMatch = entry.factory->produce(installPath);
...
bool Install::supportsImageMode(Import::ImageMode imageMode) const { return preferredImageModeOrder().contains(imageMode); }
...
DocHandlingError Install::checkoutPlatformDoc(std::unique_ptr<PlatformDoc>& returnBuffer, const QString& name)
{
    ...
    // Open document
    DocHandlingError readErrorStatus = checkoutDataDocument(returnBuffer.get(), docReader);
...
DocHandlingError Install::checkoutPlaylistDoc(std::unique_ptr<PlaylistDoc>& returnBuffer, const QString& name)
{
    // Translate to launcher doc name
...
DocHandlingError Install::commitPlatformDoc(std::unique_ptr<PlatformDoc> document)
{
    ...
    // Write
    DocHandlingError writeErrorStatus = commitDataDocument(document.get(), docWriter);
...
DocHandlingError Install::commitPlaylistDoc(std::unique_ptr<PlaylistDoc> document)
{
    // Doc should belong to this install
    assert(document->parent() == this);
